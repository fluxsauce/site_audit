    ...
    if ($this->percent != 100 || drush_get_context('DRUSH_VERBOSE')) {
      foreach ($this->checks as $check) {
        if ($check->score != AuditCheck::AUDIT_CHECK_SCORE_PASS || drush_get_context('DRUSH_VERBOSE')) {
          drush_log(dt('!result', array('!result' => $check->result)), $check->getScoreDrushLevel());
          if ($check->action) {
    ...
    if ($this->percent != 100 || drush_get_context('DRUSH_VERBOSE')) {
      foreach ($this->checks as $check) {
        if ($check->score != AuditCheck::AUDIT_CHECK_SCORE_PASS || drush_get_context('DRUSH_VERBOSE')) {
          ...
          $ret_val .= '</h3>';
          $ret_val .= '<ul>';
          $ret_val .= '<li>' . htmlspecialchars($check->result) . '</li>';
          if ($check->action) {
            $ret_val .= '<li>' . htmlspecialchars($check->action) . '</li>';
          }
          $ret_val .= '</ul>';
        }
      }
